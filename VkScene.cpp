	...
	*/
	...
	void setupDescriptors(VkDevice device, VulkanSwapChain& swapChain)
	{
		...
		*/
		...
		std::vector<VkModel::Model*> modellist = { &models.skybox, &models.scene };
	...
	VkModel::Model & GetSkyboxModel()
	{
		return models.skybox;
	}

	VkModel::Model & GetSceneModel()
	{
		return models.scene;
	}

	void loadEnvironment(std::string filename, VulkanDevice& vulkanDevice, VkQueue queue, VkPipelineCache pipelineCache)
	{
		...
		generateCubemaps(vulkanDevice, queue, pipelineCache);
	}

	void loadScene(std::string filename, VulkanDevice& vulkanDevice, VkQueue queue)
	{
		std::cout << "Loading scene from " << filename << std::endl;

		models.scene.destroy(vulkanDevice.logicalDevice);
	...
	void loadAssets(VulkanDevice& vulkanDevice, VkQueue queue, VkPipelineCache pipelineCache)
	{
		...
		loadScene(sceneFile.c_str(), vulkanDevice, queue);
		models.skybox.loadFromFile(assetpath + "models/Box/glTF-Embedded/Box.gltf", &vulkanDevice, queue);

		loadEnvironment(envMapFile.c_str(), vulkanDevice, queue, pipelineCache);
	}
	...
	void recordCommandBuffers(const Settings& settings, VkRenderPass renderPass, CommandBuffer& cmdBuffers, FrameBuffer& frameBuffers)
	{
		...
		for (decltype(cmdBuffers.Count()) i = 0; i < cmdBuffers.Count(); ++i) {
			...
			VkModel::Model &model = models.scene;
