...
namespace VkModel
{
	...
	*/
	struct Material {
		...
		VkModel::Texture *baseColorTexture;
		VkModel::Texture *metallicRoughnessTexture;
		VkModel::Texture *normalTexture;
		VkModel::Texture *occlusionTexture;
		VkModel::Texture *emissiveTexture;
		struct TexCoordSets {
			uint8_t baseColor = 0;
		...
		struct Extension {
			VkModel::Texture *specularGlossinessTexture;
			VkModel::Texture *diffuseTexture;
			...
			if (mesh) {
				...
				if (skin) {
					...
					for (size_t i = 0; i < numJoints; i++) {
						VkModel::Node *jointNode = skin->joints[i];
	...
	*/
	struct Model {
		...
		void loadNode(VkModel::Node *parent, const tinygltf::Node &node, uint32_t nodeIndex, const tinygltf::Model &model, std::vector<uint32_t>& indexBuffer, std::vector<Vertex>& vertexBuffer, float globalscale)
		{
			VkModel::Node *newNode = new Node{};
		...
		void loadTextures(tinygltf::Model &gltfModel, Vk::VulkanDevice *device, VkQueue transferQueue)
		{
			for (tinygltf::Texture &tex : gltfModel.textures) {
				...
				VkModel::TextureSampler textureSampler;
				if (tex.sampler == -1) {
		...
		void loadTextureSamplers(tinygltf::Model &gltfModel)
		{
			for (tinygltf::Sampler smpl : gltfModel.samplers) {
				VkModel::TextureSampler sampler{};
		...
		void loadAnimations(tinygltf::Model &gltfModel)
		{
			for (tinygltf::Animation &anim : gltfModel.animations) {
				...
				for (auto &samp : anim.samplers) {
					VkModel::AnimationSampler sampler{};
				...
				for (auto &source : anim.channels) {
					VkModel::AnimationChannel channel{};
		...
		void updateAnimation(uint32_t index, float time)
		{
			...
			for (auto& channel : animation.channels) {
				VkModel::AnimationSampler &sampler = animation.samplers[channel.samplerIndex];
				...
				for (size_t i = 0; i < sampler.inputs.size() - 1; i++) {
					if ((time >= sampler.inputs[i]) && (time <= sampler.inputs[i + 1])) {
						...
						if (u <= 1.0f) {
							switch (channel.path) {
							case VkModel::AnimationChannel::PathType::TRANSLATION: {
								glm::vec4 trans = glm::mix(sampler.outputsVec4[i], sampler.outputsVec4[i + 1], u);
								channel.node->translation = glm::vec3(trans);
								break;
							}
							case VkModel::AnimationChannel::PathType::SCALE: {
								glm::vec4 trans = glm::mix(sampler.outputsVec4[i], sampler.outputsVec4[i + 1], u);
								channel.node->scale = glm::vec3(trans);
								break;
							}
							case VkModel::AnimationChannel::PathType::ROTATION: {
								glm::quat q1;
