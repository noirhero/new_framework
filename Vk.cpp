...
namespace Vk
{
	...
	*/
	...
	void setupNodeDescriptorSet(VkModel::Node *node)
	{
		if (node->mesh) {
			VkDescriptorSetAllocateInfo descriptorSetAllocInfo{};
			descriptorSetAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
	...
	*/
	...
	void renderNode(VkModel::Node *node, uint32_t cbIndex, VkModel::Material::AlphaMode alphaMode)
	{
		if (node->mesh) {
	...
	void recordCommandBuffers()
	{
		...
		for (size_t i = 0; i < commandBuffers.size(); ++i) {
			...
			for (auto node : model.nodes) {
				renderNode(node, static_cast<uint32_t>(i), VkModel::Material::ALPHAMODE_OPAQUE);
			}
			// Alpha masked primitives
			for (auto node : model.nodes) {
				renderNode(node, static_cast<uint32_t>(i), VkModel::Material::ALPHAMODE_MASK);
			}
			// Transparent primitives
			// TODO: Correct depth sorting
			vkCmdBindPipeline(currentCB, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelines.pbrAlphaBlend);
			for (auto node : model.nodes) {
				renderNode(node, static_cast<uint32_t>(i), VkModel::Material::ALPHAMODE_BLEND);
			}
