...
namespace Vk {
	...
	void Geometry::CreateDescriptorPool(Main& main)
	{
	...
	void Geometry::CreateAndSetupSceneDescriptorSet(Main& main)
	{
	...
	void Geometry::CreateAndSetupMaterialDescriptorSet(Main & main)
	{
		auto device = main.GetDevice();
	...
	void Geometry::CreateAndSetupNodeDescriptorSet(Main& main)
	{
		const std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings =
		{
	...
	void Geometry::CreatePipelines(Main& main)
	{
		VkDevice device = main.GetDevice();
	...
	void Geometry::LoadScene(Main& main, std::string&& filename)
	{
		const auto tStart = std::chrono::high_resolution_clock::now();
		...
		for (auto& uniformBuffer : _sceneUniBufs)
			uniformBuffer.create(vulkanDevice, uniBufUsagFlags, uniBufMemPropertyFlags, sizeof(GeometryShaderValues));
		for (auto& uniformBuffer : _sceneShaderValueUniBufs)
			uniformBuffer.create(vulkanDevice, uniBufUsagFlags, uniBufMemPropertyFlags, sizeof(GeometryUniformData));
	}

	bool Geometry::Initialize(Main& main)
	{
		CheckToDataPath();
	...
	void Geometry::Release(VkDevice device)
	{
	...
	void Geometry::UpdateUniformDatas(const glm::mat4& view, const glm::mat4& perspective, const glm::vec3& cameraPos, const glm::vec4& lightDir)
	{
		// Geometry
	...
	void Geometry::RecordBuffers(Main& main, CommandBuffer& cmdBuffers, FrameBuffer& frameBuffers)
	{
		const Settings& settings = main.GetSettings();
	...
	void Geometry::OnUniformBufferSets(uint32_t currentBuffer)
	{
		constexpr auto uniDataSize = sizeof(GeometryUniformData);
		memcpy_s(_sceneUniBufs[currentBuffer].mapped, uniDataSize, &_sceneUniData, uniDataSize);

		constexpr auto shaderValueSize = sizeof(GeometryShaderValues);
